...
%%
function test_observer_kalman(K, L, time, theta, path, n)
    if ~exist(path, "dir")
        mkdir(path);
    end
    theta0 = theta;
    simIn = Simulink.SimulationInput('observer_kalman');
...
for i = 1:length(x_arr)
     [t, a, ang] = test_controller_initials(K, 70, x_arr(i), dx0, theta0, dtheta0, path, i + 5);
end 
...
for i = 1:length(K_arr)
    time = t_arr(i);
    [t, a, ang] = test_controller(K_arr{i}, time, theta0, path, i + 5);
end
...
vare = 0.02;
test_observer_kalman(K, L, 5, 0.2, path, 1);
